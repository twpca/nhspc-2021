// Expect: WA in subtask 3, but we failed to construct an input
#include<algorithm>
#include<numeric>
#include<queue>
...
i64 f(i64 w, int n_vertices){
   return (n_vertices+1)*w-1;
}

i64 finv(i64 w, int n_vertices){
   return (w+1)/(n_vertices+1);
}
