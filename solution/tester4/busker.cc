...
int main(){
   ...
   for(int u=0; u<n; ++u){
      for(auto vi: g0[u]){
         g[u].emplace_back(vi.first, -c[u]-accumulate(c.cbegin()+n+vi.second*k, c.cbegin()+n+(vi.second+1)*k, 0ll));
      }
   }
   ...
   vector<int> cyc0;
   vector<i64> w;
   cyc0.reserve(cyc.size()*(k+1)+1);
   w.reserve(cyc.size()*(k+1));
