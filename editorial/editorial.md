...
若 j ≥ i，從 u<sub>i</sub> 沿著 C 走到 u<sub>j</sub> 的點權重和是 s(j) - s(i-1)，
故 ⇒ 是直接結論。

若 j < i，從 u<sub>i</sub> 沿著 C 走到 u<sub>j</sub> 的點權重和變成 s(\|V(C)\|) + s(j) - s(i-1)，
又因為 s(j) ≥ 0，我們有

s(\|V(C)\|) + s(j) - s(i-1) ≥ s(\|V(C)\|) - s(i-1) ≥ 0，

這給出了 ⇐ 的證明，因此只要找出了一個非負環，就能在 O(\|C\|) 時間求出所有的好起點。
...
我們用 Bellman-Ford 演算法找出一個非負環，時間複雜度為 O(\|E(G)\|\|V(G)\|) = O(n<sup>6</sup>)。
...
給一張 n 點 m 邊的有向圖，每條邊都可以反轉，但反轉第 i 條邊需要權限 c<sub>i</sub>。
反轉複數條邊所需的權限值就是反轉邊的最大權限。
...
至於要怎麼求出方案以及判斷 x 是否是可行解，這裡提供一個方案：
...
直接枚舉所有可能的銷售都市集合，組合數不會超過 Ω(2<sup>c</sup>)。
...
- 水果數量為 Σn<sub>i</sub> 個，狀態數 O(cΣn<sub>i</sub>)
- 轉移 O(c)
...
不失一般性，假定 $h_1\le h_2\le\ldots\le h_n$。直覺告訴我們只要先把奇數項從小排到大，再把偶數項從大排到小，就能給出最佳解。
...
3. 對於所有的 $\tau \in S_i$，定義 $g(\tau; \mathbf{h})$ 為 $\max_{1\le j\le i}\left|h_{\tau((j\operatorname{mod}i)+1)}-h_{\tau(j)}\right|$。在本 subtask 中，我們要找出 $\sigma \in S_n$ 使得 $g(\sigma; \mathbf{h})$ 有最小值。
4. 定義
$\begin{align}\sigma^* = \begin{cases}1, 3, \ldots, n-2, n, n-1, n-3, \ldots, 2,&\text{if }n\text{ is odd},\\1, 3, \ldots, n-3, n-1, n, n-2, \ldots, 2,&\text{if }n\text{ is even}.\end{cases}\end{align}$
可以發現當 $n\ge2$ 時，$\sigma^*|_{n-1}$ 和 $n\gets n-1$ 時的 $\sigma^*$ 完全相同。

----

我們用數學歸納法證明 $\sigma^*$ 會給出最小的 $g(\sigma; \mathbf{h})$。對於 $n\le3$，$g(\sigma; \mathbf{h}) = h_n-h_1$ 對 $\sigma$ 是常數，$\sigma^*$ 自然會給出 $g(\sigma; \mathbf{h})$ 的最小值。假設 $n\ge4$ 並考慮把 $n$ 插入 $\sigma^*|_{n-1}$ 以得到 $\sigma^*$ 的過程。由於 $n$ 被插入 $n-2$ 和 $n-1$ 中間且 $n-1$ 和 $n-3$ 相鄰，我們知道
...
1. $g(\sigma^*|_{n-1}; \mathbf{h}) < h_n-h_{n-2} \Rightarrow g(\sigma^*; \mathbf{h}) = h_n-h_{n-2}$，故 $\sigma^*$ 最小化 $g(\sigma; \mathbf{h})$。
...
依序計算 $D_0, D_1, \ldots, D_L$，並用一個 DP 表格記錄算過的 $D_i$ 們，需時 $O(L^2)$。
...
$\begin{align}D_i&=\sum_{j=0}^{i-2}(2(i-j)-3)D_j\\&=\sum_{j=0}^{i-2}(2i-2j-3)D_j\\&=(2i-3)\sum_{j=0}^{i-2}D_j - 2\sum_{j=0}^{i-2}jD_j.\end{align}$

令 $A_i := \sum_{j=0}^iD_j$ 以及 $B_i := \sum_{j=0}^ijD_j$，則上式可以進一步寫成
...
$\begin{align}D_i = (2i-3)\sum_{j=0}^{i-2}D_j - 2\sum_{j=0}^{i-2}jD_j,\end{align}$
$\begin{align}D_{i-1} = (2i-5)\sum_{j=0}^{i-3}D_j - 2\sum_{j=0}^{i-3}jD_j.\end{align}$

將兩式相減得到

$\begin{align}D_i-D_{i-1} &= (2i-3)D_{i-2} + 2\sum_{j=0}^{i-3}D_j - 2(i-2)D_{i-2}\\ &= D_{i-2} + 2\sum_{j=0}^{i-3}D_j,\end{align}$
...
這是個線性遞迴式。搭配初始條件 $D_0 = 1, D_1 = 0, D_2 = 1$，則對於所有的 $L\ge3$，皆有

$\begin{pmatrix}D_L\\D_{L-1}\\D_{L-2}\end{pmatrix} = \begin{pmatrix}2&0&1\\1&0&0\\0&1&0\end{pmatrix}\begin{pmatrix}D_{L-1}\\D_{L-2}\\D_{L-3}\end{pmatrix} = \begin{pmatrix}2&0&1\\1&0&0\\0&1&0\end{pmatrix}^{L-2}\begin{pmatrix}1\\0\\1\end{pmatrix}.$
